...
class HabitTestCase(APITestCase):
    ...
    def test_habit_create(self):
        ...
        self.assertEqual(MyHabit.objects.last().name, 'Тест')
        self.assertEqual(MyHabit.objects.last().place, 'Дом')
        self.assertEqual(MyHabit.objects.last().date_begin, time(4, 20))
        self.assertEqual(MyHabit.objects.last().action, 'Домашние дела')
        self.assertEqual(MyHabit.objects.last().is_pleasanthabit, False)
        self.assertEqual(MyHabit.objects.last().period, 4)
        self.assertEqual(MyHabit.objects.last().award, 'Шоколадка')
