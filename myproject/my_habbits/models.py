...
class MyHabit(models.Model):
    ...
    name = models.CharField(
        max_length=400, verbose_name="Наименование привычки", blank=True, null=True
    )
    ...
    place = models.CharField(
        max_length=280, verbose_name="Место", blank=True, null=True
    )
    date_begin = models.TimeField(
        verbose_name="Дата начала выполнения", blank=True, null=True
    )
    action = models.CharField(
        max_length=280, verbose_name="Действие", blank=True, null=True
    )
    is_pleasanthabit = models.BooleanField(
        default=False, verbose_name="Приятная привычка"
    )
    is_publichabit = models.BooleanField(
    ...
    def __str__(self):
        ...
        return (
            f"Нет владельца, ответственного за привычку: {self.action} - {self.place}"
        )
